...
mkdir -p fullcircle_app/src/screens

# Crear requirements.txt
cat <<EOL > fullcircle_app/requirements.txt
docker
tkinterweb
bs4
pexpect
...
# Crear setup.py
cat <<EOL > fullcircle_app/setup.py
...
touch fullcircle_app/src/__init__.py

# Crear main.py
cat <<EOL > fullcircle_app/src/main.py
...
cat <<EOL > fullcircle_app/src/screens/home_screen.py
import tkinter as tk
import docker
...
class HomeScreen(tk.Frame):
    def __init__(self, parent, controller):
        ...
        label = tk.Label(self, text="Home Screen", font=("Arial", 24), bg="#2e2e2e", fg="#ffffff")
...
# Crear php_screen.py
cat <<EOL > fullcircle_app/src/screens/php_screen.py
import tkinter as tk
import subprocess
...
class PhpScreen(tk.Frame):
    def __init__(self, parent, controller):
        ...
        label = tk.Label(self, text="PHP Screen", font=("Arial", 24), bg="#2e2e2e", fg="#ffffff")
        label.pack(pady=10, padx=10)
        ...
        self.option_menu = tk.OptionMenu(command_frame, self.option_var, "--random", "--manual")
        ...
        self.option_menu.pack_forget()  # Ocultar el menú de opciones por defecto
        ...
        self.manual_input_frame.pack_forget()  # Ocultar el frame de entrada manual por defecto

        self.run_command()  # Ejecutar el comando por defecto al iniciar

    def update_options(self, selected_command):
    ...
    def create_array_input(self):
        ...
        def submit_array():
            array = [int(entry.get()) for entry in self.array_entries if entry.get().isdigit()]
            self.execute_manual_command(array)

        add_button = tk.Button(self.manual_input_frame, text="Add Number", command=add_entry, bg="#3e3e3e", fg="#ffffff", font=("Arial", 20))
        add_button.pack(pady=10)

        submit_button = tk.Button(self.manual_input_frame, text="Submit", command=submit_array, bg="#3e3e3e", fg="#ffffff", font=("Arial", 20))
    ...
    def create_orders_input(self):
        ...
        def add_order():
            ...
            id_entry = tk.Entry(frame, font=("Arial", 20))
            id_entry.pack(side=tk.LEFT, padx=5)
            status_entry = tk.Entry(frame, font=("Arial", 20))
            status_entry.pack(side=tk.LEFT, padx=5)
            total_entry = tk.Entry(frame, font=("Arial", 20))
            total_entry.pack(side=tk.LEFT, padx=5)

            self.orders_entries.append((id_entry, status_entry, total_entry))

        def submit_orders():
            orders = []
            for id_entry, status_entry, total_entry in self.orders_entries:
                if id_entry.get().isdigit() and total_entry.get().isdigit():
                    orders.append({
                        "id": int(id_entry.get()),
                        "status": status_entry.get(),
                        "total": float(total_entry.get())
                    })
            self.execute_manual_command(orders)

        add_button = tk.Button(self.manual_input_frame, text="Add Order", command=add_order, bg="#3e3e3e", fg="#ffffff", font=("Arial", 20))
        add_button.pack(pady=10)

        submit_button = tk.Button(self.manual_input_frame, text="Submit", command=submit_orders, bg="#3e3e3e", fg="#ffffff", font=("Arial", 20))
        submit_button.pack(pady=10)
    ...
    def create_transactions_input(self):
        ...
        def add_transaction():
            ...
            self.transactions_entries.append((id_entry, amount_entry, date_entry, category_entry))

        def submit_transactions():
            transactions = []
            for id_entry, amount_entry, date_entry, category_entry in self.transactions_entries:
                if id_entry.get().isdigit() and amount_entry.get().isdigit() and date_entry.get():
                    transactions.append({
                        "id": int(id_entry.get()),
            ...
            self.execute_manual_command(transactions)
        ...
        submit_button = tk.Button(self.manual_input_frame, text="Submit", command=submit_transactions, bg="#3e3e3e", fg="#ffffff", font=("Arial", 20))
        submit_button.pack(pady=10)

    def execute_manual_command(self, data):
        self.terminal.config(state=tk.NORMAL)
        self.terminal.delete(1.0, tk.END)  # Limpiar la consola
        try:
            # Aquí puedes ejecutar el comando con los datos manuales
            # Por ejemplo, puedes pasar los datos a un script de Python que los procese
            result = subprocess.run(["docker", "exec", "php_container", "sh", "-c", f"fullcircle {self.command_var.get()} --manual"], input=str(data), capture_output=True, text=True)
...
cat <<EOL > fullcircle_app/src/screens/laravel_screen.py
import tkinter as tk
from tkinter import ttk
import requests
import json
...
class LaravelScreen(tk.Frame):
    def __init__(self, parent, controller):
        ...
        label = tk.Label(self, text="Laravel Screen", font=("Arial", 24), bg="#2e2e2e", fg="#ffffff")
        label.pack(pady=10, padx=10)
...
cat <<EOL > fullcircle_app/src/screens/wordpress_screen.py
import tkinterweb
import tkinter as tk
...
cat <<EOL > fullcircle_app/src/screens/mysql_screen.py
import tkinter as tk
...
class MysqlScreen(tk.Frame):
    def __init__(self, parent, controller):
        ...
        lorem_ipsum = (
            ...
            "Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea#!/bin/bash
